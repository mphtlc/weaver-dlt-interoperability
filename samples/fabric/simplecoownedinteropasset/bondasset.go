...
func (s *SmartContract) InitBondSharedAssetLedger(ctx contractapi.TransactionContextInterface) error {
	assets := []BondAsset{
		{Type: "t1", ID: "a01", Issuer: "Treasury" , CoOwners: []string {""}, FaceValue: 300,
			 MaturityDate: time.Date(2022, time.April, 1, 12, 0, 0, 0, time.UTC)},
			 {Type: "t1", ID: "a02", Issuer: "Treasury" , CoOwners: []string {""}, FaceValue: 400,
...
func (s *SmartContract) CreateSharedAsset(ctx contractapi.TransactionContextInterface, assetType, id string, coOwners []string, issuer string, faceValue int, maturityDate string) error {
	if assetType == "" {
...
func (s *SmartContract) ReadSharedAsset(ctx contractapi.TransactionContextInterface, assetType, id string, isUpdateOwnerContext bool) (*BondAsset, error) {
	...
	if !isUpdateOwnerContext {
		...
		if len(asset.CoOwners) != 0 && !isElementOf(asset.CoOwners, owner) {
...
func (s *SmartContract) DeleteAsset(ctx contractapi.TransactionContextInterface, assetType, id string) error {
	...
	if !checkAccessToSharedAsset(s, ctx, assetJSON) {
...
func isElementOf(strings []string, element string) bool {
...
// isCallerAssetCoOwner returns true only if the invoker of the transaction is also a co-owner of the asset
func isCallerAssetCoOwner(ctx contractapi.TransactionContextInterface, asset *BondAsset) bool {
	caller, err := getECertOfTxCreatorBase64(ctx)
	if err != nil {
		fmt.Println(err.Error())
		return false
	}
	return len(asset.CoOwners) != 0 && isElementOf(asset.CoOwners, caller)
}

// isBondAssetLocked returns true only if the asset is presently locked
func isBondSharedAssetLocked(s *SmartContract, ctx contractapi.TransactionContextInterface, asset *BondAsset) bool {
	bondAssetAgreement := &common.AssetExchangeAgreement{
		Type: asset.Type,
		Id: asset.ID,
		Recipient: "*",
		Locker: strings.Join(asset.CoOwners, ","),
	}
	...
	locked, err := s.IsSharedAssetLocked(ctx, bondAssetAgreementProto64)
	if err != nil {
		fmt.Println(err.Error())
		return false
	}
...
func isBondSharedAssetLockedForMe(s *SmartContract, ctx contractapi.TransactionContextInterface, asset *BondAsset) bool {
	bondAssetAgreement := &common.AssetExchangeAgreement{
		Type: asset.Type,
		Id: asset.ID,
		Recipient: "",
...
func checkAccessToSharedAsset(s *SmartContract, ctx contractapi.TransactionContextInterface, assetJSON []byte) bool {
	var asset BondAsset
	err := json.Unmarshal(assetJSON, &asset)
	if err != nil {
		fmt.Println(err.Error())
...
func (s *SmartContract) IsAssetReleased(ctx contractapi.TransactionContextInterface, assetType, id string) (bool, error) {
	asset, err := s.ReadSharedAsset(ctx, assetType, id, false)
	if err != nil {
		return false, err
	}
	currDate := time.Now()
...
func (s *SmartContract) UpdateCoOwners(ctx contractapi.TransactionContextInterface, assetType, id string, newCoOwners []string) error {
	asset, err := s.ReadSharedAsset(ctx, assetType, id, true)
	if err != nil {
		return err
	}

	asset.CoOwners = newCoOwners
	assetJSON, err := json.Marshal(asset)
	if err != nil {
		return err
	}
...
func (s *SmartContract) GetMySharedAssets(ctx contractapi.TransactionContextInterface) ([]*BondAsset, error) {
	owner, err := getECertOfTxCreatorBase64(ctx)
	if err != nil {
		return nil, err
	}
	...
	for _, asset := range assets {
		if isElementOf(asset.CoOwners, owner) {
